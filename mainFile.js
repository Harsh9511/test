// Plugin Name : phononreversefeed
// Desc : medical phononreversefeed 
...
class phononreversefeed extends imodule {
    async post_root(parts) {
        ...
        try {
            ...
            let racdata = [];
            let iteration_end = crosell_flag ? 2 : 1 ;
            console.log({armaancrosell_flag:{crosell_flag,iteration_end}})
        for(let ite = 0; ite < iteration_end ; ite++){
            
            if(ite == 1){
               // return this.sendSuccess(ite)
            ...
            if (responsedata.status == 0) {
                ...
                responsedata = JSON.parse(resData);
                console.log({armaanclaim:{ite,responsedata}})
                if (responsedata.status == 0) {
                    ...
                    responsedata = JSON.parse(resData);
                    console.log({armaanaAssign:{ite,responsedata}})
                    ...
                    if (responsedata.status == 0) {
                        ...
                        responsedata = JSON.parse(resData);
                        console.log({armaanCounter:{ite,responsedata}})
                        if (responsedata.status == 0) {
                            ...
                            responsedata = JSON.parse(resData);
                            console.log({armaanProposal:{ite,responsedata}})
                            ...
                            let pace_url;
                            var product_name = responsedata.data[0].proposal.data.product_name;
                            if (responsedata.data[0].proposal.data.nstp_final_status == "Reject") {
                                if(product_name != "Criti-Medicare"){
                                    //partner Pace API
                                    let policyId = responsedata.data[0].policy_id;
                                    let product_code = responsedata.data[0].proposal.data.gc_product_code
                                    ...
                                    let sql = ""
                                    try{
                                    _paceres = await this.__call_node_method(plugin_url, [this.req, this.res], [policyId, serviceId,id])
                                    sql = `insert into health_partner_integration(Proposal_No,Policy_ID,Integration,add_info_1,api_response,product_code) values (?,?,?,?,?,?)`
                                    _paceres = await db.exec(sql,[responsedata.data[0].proposal_no,policyId,serviceId,id,JSON.stringify(_paceres),product_code])
                                    }catch(ex){
                                        ex = typeof ex != 'string' ? JSON.stringify(ex) : ex;
                                        sql = `insert into health_partner_integration(Proposal_No,Policy_ID,Integration,add_info_1,api_response,product_code) values (?,?,?,?,?,?)`
                                       _paceres = await db.exec(sql,[responsedata.data[0].proposal_no,policyId,serviceId,id,ex,product_code])
                                    }
                                }else{
                                    // pace sync for criti medicare
                                    pace_url = `${base_url}/api/v1/cc_common/integratetopace?policy_id=` + policy.policy_id;
                                    let pace_head = {
                                        'in-auth-token': this.req.headers["in-auth-token"],
                                        'Content-Type': 'application/json'
                                    };
                                    let pace_req = {
                                        'in-auth-token': this.req.headers["in-auth-token"]
                                    };
                                    let pace_data = await utils.apost(pace_url, pace_req, pace_head);
                                }
                        
                                await this.submit_log(proposalno, "Error - Proposal rejected", url + " " + JSON.stringify(updated_json), responsedata);
                                // Added below change for cross sell on 11/01/2022 for RH-549 by Armaan
                                if(!crosell_flag){
                                    ...
                                    racdata.push({
                                        ...
                                        "ipdsstatus": "Your Proposal is Rejected by BRE based on Question details"
                                    });
                                    if(ite == 0) continue;
                                ...
                                if (responsedata.status == 0) {
                                    ...
                                    if (responsedata.status == 0) {
                                        ...
                                        if (responsedata.status == 0) {
                                            ...
                                            .catch(err=>{
                                                console.log({armaanFinalAssign:{ite,err}})
                                            })

                                            responsedata = JSON.parse(resData);
                                            console.log({armaanFinalAssign:{ite,responsedata,proposalno}})
                                            if (responsedata.status == 0) {
                                                // For success
                                                if(product_name != "Criti-Medicare"){
                                                    let id = await this.random_id()     
                                                    ...
                                                    try{
                                                        ...
                                                        sql = `insert into health_partner_integration(Proposal_No,Policy_ID,Integration,add_info_1,api_response,product_code) values (?,?,?,?,?,?)`
                                                        await db.exec(sql,[policy.proposal_no,policyId,serviceId,id,JSON.stringify(resp_pace),product_code])
                                                    }catch(ex){
                                                        ex = typeof ex != 'string' ? JSON.stringify(ex) : ex
                                                        sql = `insert into health_partner_integration(Proposal_No,Policy_ID,Integration,add_info_1,api_response,product_code) values (?,?,?,?,?,?)`
                                                        await db.exec(sql,[policy.proposal_no,policyId,serviceId,id,ex,product_code])
                                                    }
                                                }else{
                                                    // pace sync for criti medicare
                                               ...
                                               // let _paceres = await this.integratetopace(proposalno);
                                                //console.log({armaanPace:{ite,_paceres}})
                                                ...
                                                if(!crosell_flag){
                                                    ...
                                                    let s_log = await this.submit_log(proposalno, "Success", racdata, "Proposal submitted successfully");
                                                    console.log({armaanRac:{ite,racdata,s_log}})
                                                    if(ite == 1) return this.sendSuccess(racdata);
                                                }
                                            } else {
